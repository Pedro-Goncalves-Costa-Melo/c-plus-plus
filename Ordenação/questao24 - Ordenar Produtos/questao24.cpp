...
void SelectionSort(Produto vet[], int tam)
{
...
int main(){
    ...
    float limBaixo;
    float limAlto;
    cin>>limBaixo>>limAlto;

    SelectionSort(vet,n);

    for (int i = 0; i < n; i++)
    {
         if (vet[i].preco<limAlto && vet[i].preco>limBaixo)
        {
            cout<<vet[i].identificador<<" \""<<vet[i].descricao<<"\" "<<vet[i].preco<<endl;
        }
